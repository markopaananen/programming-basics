...
namespace task4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nYour series was {0}", strNumbers);
            Console.WriteLine("\nThe biggest number was {0} and it was {1}. number in the serie.", maxInteger, maxIntegerPosition);
            ...
            int MaxOfTen(out string numbersToPrint, out int maxIntegerPositionToPrint)
            {
                ...
                for (int i = 1; i <= 10; i++)
                {
                    ...
                    while ((!int.TryParse(userInput, out number)) || (number < 0))
                    {
                        ...
                        Console.WriteLine("Wrong number, must be positive!");
                        Console.Write("{0}. ", i);
                        // Store user input to a variable
